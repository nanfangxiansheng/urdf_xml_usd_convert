...
def main():
    ...
    root_dir = Path("/home/blackbird/GYH/out_pm")
    ...
    for category_dir in root_dir.iterdir():
        if category_dir.is_dir():
            ...
            for obj_dir in category_dir.iterdir():
                if obj_dir.is_dir():
                    # 构建输入文件路径
                    input_xml = os.path.abspath(str(obj_dir) + "/model_fixed1.xml")
                    input_xml_path=Path(str(obj_dir) + "/model_fixed1.xml")
                    # 检查文件是否存在且非空
                    if input_xml_path.exists() and input_xml_path.stat().st_size > 0:
                        # 构建输出文件路径
                        output_usd =os.path.abspath( str(obj_dir) + "/model_fixed1.usda")
